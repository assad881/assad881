...
KX.viewsPublic = (function () {
  ...
  function home() {
    ...
    const how =
      ...
      [['1', 'حدّد موقعك والمادة', 'اختر موقع المشروع والمادة والكمية بالمتر المكعب أو بعدد الشاحنات.'],
  ...
  function how() {
    const steps = [
      ...
      ['اختيار المادة والكمية', 'اختر المادة والمورد، وأدخل الكمية بالمتر المكعب أو بعدد الشاحنات.'],
  ...
  async function suppliers() {
    ...
    const cards = rows.map(function (s) {
      ...
      const items = list.slice(0, 5).map((p) =>
        ...
        '<b class="mono">' + e(KX.util.money(p.price_per_unit)) + ' / م³</b></div>').join('');
      return '<div class="card"><div class="row row--between">' +
        ...
        '<div class="row gap-sm mb">' + KX.ui.badge('طاقة تحميل ' + KX.util.fmtNum(s.loading_capacity_per_day) + ' م³/يوم') +
    ...
    L().renderPublic('<h1>الموردون المعتمدون</h1>' +
      '<p>كسارات ومصانع مسجّلة في المنصة داخل محافظة الظاهرة. '
      + 'الأسعار المعروضة لكل متر مكعب من باب الكسارة، قبل النقل والرسوم والضريبة.</p>' +
      KX.ui.alert('قائمة الأسعار مصدرها قائمة أسعار منتجات الكسارة الرسمية للمورد، '
        + 'وتخضع للتعديل من الإدارة. النقل والضريبة يُحسبان بندًا مستقلًا بعد تحديد الموقع.', 'info', 'ℹ️') +
  ...
  function faq() {
    const items = [
      ...
      ['كيف يُحسب عدد الشاحنات؟',
       'يقسم النظام الكمية المطلوبة على حمولة نوع الشاحنة الذي اخترته ويقرّب لأعلى. '
       + 'مثال: 36 م³ بشاحنة 18 م³ = رحلتان. والحد الأدنى للتوصيل شاحنة واحدة (18 م³).'],
      ['هل السعر شامل النقل؟',
